import os
import re

# data directory
DATA_DIR = "./data"

# files to store data in
CHARITY_CSV = os.path.join(DATA_DIR, "charities_active.csv")
UKCAT_FILE = os.path.join(DATA_DIR, "ukcat.csv")
ICNPTSO_CSV = os.path.join(DATA_DIR, "icnptso.csv")
ICNPTSO_MODEL = os.path.join(DATA_DIR, "icnptso_ml_model.pkl")
UKCAT_ML_OVR_MODEL = os.path.join(DATA_DIR, "ukcat_ml_ovr.pkl")
UKCAT_ML_HYBRID_OVR_MODEL = os.path.join(DATA_DIR, "ukcat_ml_hybrid_ovr.pkl")
UKCAT_ML_HYBRID_CONFIG = os.path.join(DATA_DIR, "ukcat_ml_hybrid.json")
UKCAT_DEV_FILE = os.path.join(DATA_DIR, "ukcat_dev.csv")
UKCAT_FINAL_TEST_FILE = os.path.join(DATA_DIR, "ukcat_final_test.csv")
UKCAT_HOLDOUT_SPLIT_FILE = os.path.join(DATA_DIR, "ukcat_holdout_split.json")
UKCAT_BEST_DEV_CONFIG = os.path.join(DATA_DIR, "ukcat_best_dev_config.json")
SAMPLE_FILE = os.path.join(DATA_DIR, "sample.csv")
TOP2000_FILE = os.path.join(DATA_DIR, "top2000.csv")
MANUAL_ADJUSTMENTS_FILE = os.path.join(DATA_DIR, "manual_adjustments.csv")

# regexes for use in machine learning models and text cleaning
REPLACE_BY_SPACE_RE = re.compile("[/(){}\[\]\|@,;]")
BAD_SYMBOLS_RE = re.compile("[^0-9a-z #+_]")
ADDITIONAL_STOPWORDS = [
    "trust",
    "fund",
    "charitable",
    "charity",
]

# machine learning settings
ML_RANDOM_STATE = 42
ML_TEST_TRAIN_SIZE = 0.2
ML_DEFAULT_FIELDS = ["name", "activities"]
ML_CATEGORY_FIELD = "ICNPTSO"

# bands for summary display
INCOME_BINS = [-1, 10000, 100000, 250000, 500000, 1000000, 10000000, float("inf")]
INCOME_BIN_LABELS = [
    "Under £10k",
    "£10k - £100k",
    "£100k - £250k",
    "£250k - £500k",
    "£500k - £1m",
    "£1m - £10m",
    "Over £10m",
]

# location of charity download files
######################################

# Charity Commission for England and Wales
CCEW_CHARITY_FILE = "https://ccewuksprdoneregsadata1.blob.core.windows.net/data/txt/publicextract.charity.zip"
CCEW_GD_FILE = (
    "https://ccewuksprdoneregsadata1.blob.core.windows.net/data/txt/publicextract.charity_governing_document.zip"
)
CCEW_PARTA_FILE = (
    "https://ccewuksprdoneregsadata1.blob.core.windows.net/data/txt/publicextract.charity_annual_return_parta.zip"
)

# Office of Scottish Charity Regulator
OSCR_ACTIVE = "https://www.oscr.org.uk/download/charity-register"
OSCR_INACTIVE = "https://www.oscr.org.uk/download/charity-former-register"

# Charity Commission for Northern Ireland
CCNI_DATA = "https://www.charitycommissionni.org.uk/umbraco/api/charityApi/ExportSearchResultsToCsv/?include=Linked&include=Removed"
CCNI_ACTIVITIES_CSV = "./data/ccni_activities.csv"
