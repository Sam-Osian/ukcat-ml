...
def create_ukcat_hybrid_model(
    ...
    _train_ukcat_ovr_artifact(
        ...
        sgd_loss=cfg.get("sgd_loss"),
        sgd_alpha=(float(cfg["sgd_alpha"]) if cfg.get("sgd_alpha") is not None else None),
        fallback_mode=str(cfg.get("fallback_mode", "top_k")),
        fallback_min_score=float(cfg.get("fallback_min_score", 0.0)),
